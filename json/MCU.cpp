...
#define TOKEN_PRINT(t) PrintToken(t)
...
static void PrintToken( jsmntok_t Token ) {

    CConsole::WriteString( SPSTR( "start: " ) );
    CConsole::WriteString( utoa_fast_div( Token.start, buffer ) );

    CConsole::WriteString( SPSTR( ", end: " ) );
    CConsole::WriteString( utoa_fast_div( Token.end, buffer ) );

    CConsole::WriteString( SPSTR( ", type: " ) );

    switch ( Token.type ) {

        case JSMN_OBJECT: {

            CConsole::WriteString( SPSTR( "JSMN_OBJECT" ) );
            break;
        }

        case JSMN_PRIMITIVE: {

            CConsole::WriteString( SPSTR( "JSMN_PRIMITIVE" ) );
            break;
        }

        case JSMN_STRING: {

            CConsole::WriteString( SPSTR( "JSMN_STRING" ) );
            break;
        }

        case JSMN_ARRAY: {

            CConsole::WriteString( SPSTR( "JSMN_ARRAY" ) );
            break;
        }

    }

    CConsole::WriteString( SPSTR( ", size: " ) );
    CConsole::WriteString( utoa_fast_div( Token.size, buffer ) );
...
int test_empty() {
    ...
    text = "{}";    
    result = jsmn.Parse( & parser, text, tokens, sizeof( tokens ) );

    check( result == JSMN_SUCCESS );
    check( tokens[0].type == JSMN_OBJECT );
    check( tokens[0].start == 0 && tokens[0].end == 2 );

    PrintToken( tokens[0] );

    //text = "[]";
    //jsmn_init( & parser );
    //result = jsmn_parse( & parser, text, tokens, sizeof( tokens ) );
    //
    //check( result == JSMN_SUCCESS );
    //check( tokens[0].type == JSMN_ARRAY );
    //check( tokens[0].start == 0 && tokens[0].end == 2 );
    ...
    //check( result == JSMN_SUCCESS);
    //check( tokens[0].type == JSMN_OBJECT && tokens[0].start == 0 && tokens[0].end == 8 );
    //check( tokens[1].type == JSMN_STRING && tokens[1].start == 2 && tokens[1].end == 3 );
    //check( tokens[2].type == JSMN_ARRAY && tokens[2].start == 5 && tokens[2].end == 7 );
    ...
    //check( result == JSMN_SUCCESS);
    //check( tokens[0].type == JSMN_ARRAY && tokens[0].start == 0 && tokens[0].end == 7 );
    //check( tokens[1].type == JSMN_OBJECT && tokens[1].start == 1 && tokens[1].end == 3 );
    //check( tokens[2].type == JSMN_OBJECT && tokens[2].start == 4 && tokens[2].end == 6 );
...
void Info() {
    ...
    CConsole::WriteString( SPSTR( "Тестовый проект json, версия " ), CConsole::cp1251 );
...
void CMCU::USARTInit(){
    ...
    //           00000000 - Initial Value
    UCSRB = BIN8(00001000); // BIN8() не зависит от уровня оптимизации
    //           ||||||||
    //           76543210
