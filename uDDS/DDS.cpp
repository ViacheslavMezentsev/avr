///////////////////////////////////////////////////////////
//  DDS.cpp
//  Implementation of the Class CDDS
//  Created on:      06-окт-2013 23:23:27
//  Original author: Слава
///////////////////////////////////////////////////////////

#include "Defines.h"
#include "Configuration.h"
#include "EFS.h"
#include "DDS.h"


// -=[ Внешние ссылки ]=-


// -=[ Постоянные во флеш-памяти ]=-

// Таблицы сигналов
// sine
#ifdef __ICCAVR__
__flash uint8_t SineWave[ 256 ] @ 0x3A00 = {
#elif defined __GNUC__
uint8_t _SineWave[ 256 ] __attribute__ ((section (".MySection1"))) = {
#endif
    
};
#ifdef __GNUC__
FU08T_PTR SineWave(_SineWave);
#endif

// square wave
#ifdef __ICCAVR__
__flash uint8_t SquareWave[ 256 ] @ 0x3B00 = {
#elif defined __GNUC__
uint8_t _SquareWave[ 256 ] __attribute__ ((section (".MySection2"))) = {
#endif
    
};
#ifdef __GNUC__
FU08T_PTR SquareWave(_SquareWave);
#endif

// sawtooth wave
#ifdef __ICCAVR__
__flash uint8_t SawToothWave[ 256 ] @ 0x3C00 = {
#elif defined __GNUC__
uint8_t _SawToothWave[ 256 ] __attribute__ ((section (".MySection3"))) = {
#endif
    
};
#ifdef __GNUC__
FU08T_PTR SawToothWave(_SawToothWave);
#endif

// reverse sawtooth wave
#ifdef __ICCAVR__
__flash uint8_t RewsawToothWave[ 256 ] @ 0x3D00 = {
#elif defined __GNUC__
uint8_t _RewsawToothWave[ 256 ] __attribute__ ((section (".MySection4"))) = {
#endif
    
};
#ifdef __GNUC__
FU08T_PTR RewsawToothWave(_RewsawToothWave);
#endif

// triangle wave
#ifdef __ICCAVR__
__flash uint8_t TriangleWave[ 256 ] @ 0x3E00 = {
#elif defined __GNUC__
uint8_t _TriangleWave[ 256 ] __attribute__ ((section (".MySection5"))) = {
#endif
    
};
#ifdef __GNUC__
FU08T_PTR TriangleWave(_TriangleWave);
#endif

// ECG wave
#ifdef __ICCAVR__
__flash uint8_t ECGWave[ 256 ] @ 0x3F00 = {
#elif defined __GNUC__
uint8_t _ECGWave[ 256 ] __attribute__ ((section (".MySection6"))) = {
#endif
    73,74,75,75,74,73,73,73,73,72,71,69,68,67,67,67,
    68,68,67,65,62,61,59,57,56,55,55,54,54,54,55,55,
    55,55,55,55,54,53,51,50,49,49,52,61,77,101,132,
    169,207,238,255,254,234,198,154,109,68,37,17,5,
    0,1,6,13,20,28,36,45,52,57,61,64,65,66,67,68,68,
    69,70,71,71,71,71,71,71,71,71,72,72,72,73,73,74,
    75,75,76,77,78,79,80,81,82,83,84,86,88,91,93,96,
    98,100,102,104,107,109,112,115,118,121,123,125,
    126,127,127,127,127,127,126,125,124,121,119,116,
    113,109,105,102,98,95,92,89,87,84,81,79,77,76,75,
    74,73,72,70,69,68,67,67,67,68,68,68,69,69,69,69,
    69,69,69,70,71,72,73,73,74,74,75,75,75,75,75,75,
    74,74,73,73,73,73,72,72,72,71,71,71,71,71,71,71,
    70,70,70,69,69,69,69,69,70,70,70,69,68,68,67,67,
    67,67,66,66,66,65,65,65,65,65,65,65,65,64,64,63,
    63,64,64,65,65,65,65,65,65,65,64,64,64,64,64,64,
    //64,64,65,65,65,66,67,68,69,71,72,73
};
#ifdef __GNUC__
FU08T_PTR ECGWave(_ECGWave);
#endif


// -=[ Переменные в ОЗУ ]=-

// Таблица сигналов.
FU08T_PTR Signals[] = {

    SineWave,
    SquareWave,
    TriangleWave,
    SawToothWave,
    RewsawToothWave,
    ECGWave

};

// Variables to control TDA7313
SETTINGS Settings;
FILEINFO FileInfo;


/***********************
*  Р Е А Л И З А Ц И Я
*  ~~~~~~~~~~~~~~~~~~~
************************/


void CDDS::Initialization() {

    if ( CEFS::GetFileInfo( 1, 0x10, & FileInfo ) == 1 ) {

        Settings.Mode = sigSineWave;

        // Initial frequency 1kHz.
        Settings.Frequency1 = 0xE8;
        Settings.Frequency2 = 0x03;
        Settings.Frequency3 = 0x00;

        // Записываем настройки в EEPROM.
        CEFS::CreateFile( & FileInfo, 0x10, "Настройки" );
        CEFS::WriteFile( & FileInfo, ( uint8_t * ) & Settings, 0, sizeof( Settings ) );

    }

    // Считываем настройки из EEPROM.
    CEFS::ReadFile( & FileInfo, ( uint8_t * ) & Settings, 0, sizeof( Settings ) );

    Settings.Frequency = ( uint32_t ) ( Settings.Frequency3 ) << 16;
    Settings.Frequency |= ( uint32_t ) ( Settings.Frequency2 ) << 8;
    Settings.Frequency |= ( uint32_t ) Settings.Frequency1;

    Settings.Accumulator = Settings.Frequency / RESOLUTION;
    Settings.Flag = 0;

    // Default 1MHz HS signal freq.
    Settings.HSFrequency = 1;
    Settings.DeltaFreq = 100;

    //------------init DDS output-----------

    // Set DDS R2R port as output.
    DDS_DDR_R2R = 0xFF;

    // Set initial zero values.
    DDS_PORT_NAME_R2R = 0x00;

    //-------------set ports pins for buttons----------
    // Не используется.

    //---------set ports pins for HS output---------
    sbi( DDS_DDR_HS, DDS_PIN_HS );

    //-----------Menu init--------------

    // Default signal is off.
    Settings.On = 0;

    //Menu_Update(SG.ON);

}


/*DDS signal generation function
Original idea is taken from
http://www.myplace.nu/avr/minidds/index.htm
small modification is made - added additional command which
checks if CPHA bit is set in SPCR register if yes - exit function
*/
void CDDS::Run( PGM_P signal, uint8_t ad2, uint8_t ad1, uint8_t ad0 ) {

#ifdef __GNUC__

    asm volatile (

        "eor r18, r18 	;r18<-0"	"\n\t"
        "eor r19, r19 	;r19<-0"	"\n\t"
        "1:"						"\n\t"
        "add r18, %0	;1 cycle"			"\n\t"
        "adc r19, %1	;1 cycle"			"\n\t"	
        "adc %A3, %2	;1 cycle"			"\n\t"
        "lpm 			;3 cycles" 	"\n\t"
        "out %4, __tmp_reg__	;1 cycle"	"\n\t"
        "sbis %5, 2		;1 cycle if no skip" "\n\t"
        "rjmp 1b		;2 cycles. Total 10 cycles"	"\n\t"
        :
        :"r" (ad0), "r" (ad1), "r" (ad2), "e" (signal),
            "I" (_SFR_IO_ADDR(DDS_PORT_NAME_R2R)), "I" (_SFR_IO_ADDR(SPCR))
        :"r18", "r19"
    );

#elif defined( __ICCAVR__ )

#endif

}

