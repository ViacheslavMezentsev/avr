...
/*        sbi( SPCR, CPHA );
        ...
        switch ( Settings.Mode ) {
            ...
            case sigSineWave: Settings.Mode = sigSquareWave; break;
            case sigSquareWave: Settings.Mode = sigTriangleWave; break;
            case sigTriangleWave: Settings.Mode = sigSawToothWave; break;
            case sigSawToothWave: Settings.Mode = sigRewsawToothWave; break;
...
void CMCU::OnUSARTRxComplete( uint8_t data ){
    ...
    // Обнуляем флаг в настройках DDS.
    Settings.Flag = 0;

    // Используем бит CPHA как признак для остановки DDS.
    SPCR |= ( 1 << CPHA );
    ...
    if ( !FIFO_IS_FULL( uart_rx_fifo ) ) {
        ...
        TCNT2 = ( uint8_t ) ( 3 * ( BAUD / 10000UL ) * ( 0xFF - F_CPU / 64000UL ) );
